...
const Customers = () => {
  ...
  const addDummyData = async () => {
    for (const customer of dummyData) {
      const newCustomerRef = ref(db, 'customers/' + Date.now() + Math.random().toString(36).substr(2, 9));
  ...
  return (
    <div className={styles.container}>
      <h1 style={{fontSize:'25px',marginBottom:'5px',textDecoration:'underLine'}}>SHREE RENUKA TRADERS</h1>
      <h2 style={{fontSize:'20px',marginBottom:'20px'}}>Customers</h2>
      <input
        type="text"
        value={searchQuery}
      ...
      <div className={styles.count}>
        <span style={{marginRight:'30px'}}>Total Customers: <span style={{color:'red'}}>{totalCustomers}</span></span>
        <span>Filtered Customers: <span style={{color:'blue'}}>{filteredCustomerCount}</span> </span>
      </div>
      <button onClick={() => router.push('/add-customer')} className={styles.addButton}>Add Customer</button>
      <div className={styles.customerList}>
        {filteredCustomers.map(([customerId, customerData]) => (
          <div key={customerId} className={styles.customerCard} onClick={() => handleCustomerClick(customerId)}>
            ...
            <p><strong>Alternate Mobile Number:</strong> {customerData.alternateMobileNumber}</p>
            <p><strong>Account Balance:</strong> {customerData.accountBalance}</p>
            <button onClick={() => handleUpdate(customerId, { ...customerData, name: customerData.name + " Updated" })}>Update</button>
            <button onClick={() => handleDelete(customerId)}>Delete</button>
          </div>
        ))}
      </div>
    </div>
